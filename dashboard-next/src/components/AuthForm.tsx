...
export default function AuthForm({ mode }: AuthFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // or signUp for signup mode
    console.log("Auth submit:", { email, password, mode });
    setTimeout(() => setLoading(false), 1000);
  };

  const handleOAuthLogin = async (provider: "github" | "google") => {
    // Supabase OAuth integration placeholder
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-md border border-white/10 bg-obsidian p-8"
    >
      ...
      <p className="mb-8 font-mono text-xs text-paper/50">
        {isLogin
          ? "Enter your credentials to access the swarm"
          : "Join the distributed intelligence network"}
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="agent@swarm.network"
            required
        ...
        <button
          type="submit"
          disabled={loading}
          className="group relative mt-6 w-full overflow-hidden border border-azure bg-azure px-4 py-3 font-mono text-xs uppercase tracking-widest text-obsidian transition-all hover:bg-azure/90 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {loading ? (
